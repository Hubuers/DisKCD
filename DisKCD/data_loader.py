...
class TrainDataLoader(object):
    ...
    def __init__(self,batch_size):
        ...
        with open(config_file) as i_f:
            i_f.readline()
            student_n, exercise_n, knowledge_n,unknowledge_n = i_f.readline().split(',')
        self.knowledge_dim = int(knowledge_n)+int(unknowledge_n)
...
class ValTestDataLoader(object):
    def __init__(self, d_type='predict'):
        ...
        if d_type == 'test':
            ...
            data_file='../data/ASSIST/untestdata.json'
        config_file = './config.txt'
        with open(data_file, encoding='utf8') as i_f:
            self.data = json.load(i_f)
        with open(config_file) as i_f:
            i_f.readline()
            _, _, knowledge_n,unknowledge_n = i_f.readline().split(',')
            self.knowledge_dim = int(knowledge_n)+int(unknowledge_n)
